...
class Simulation(metaclass=Singleton):
    ...
    def next_tick(self, selected_vessel):
        ...
        for x in self.vessels:
            for y in x['vessels']:
                ...
                results = Calculation.calculate_destination(y["distance_per_tick"], math.radians(y['bearing']), math.radians(y['lat']), math.radians(y['lon']))
                y['lat'] = results[0]
                y['lon'] = results[1]
    ...
    def generate(self, coordinates_1: list[float], coordinates_2: list[float], current_route_index, density: int = 5, noise: float = 0.05):
        ...
        for _ in range(density):
            ...
            type = Simulation().generate_type();
            metadata = {
                "vessel_type": type["name"],
                "mmsi": Simulation().mmsi,
                "course": rand_point[2],
                "bearing": rand_point[2],
                "heading": rand_point[2],
                "distance_per_tick":  type["speed"]["value"] / self.tick_rate,
                "lon": rand_point[0],
                "lat": rand_point[1],
                "aisRange": type["aisRange"]["value"],
                "aisBroadcastInterval": type["aisBroadcastInterval"]["value"],
    ...
    def generate_type(self):
        type = random.choice(self.types["vesselTypes"])
        return type