...
POSSIBLE_TYPES = [{"type_name":"A", "AIS_range": 10**4, "broadcast_interval": 360}]
...
class Vessel(metaclass=Singleton):
    ...
    def select_vessel(self, mmsi: int):
        ...
        closest = Util.find_in_range(self.vessels_ordered_by_latitude, self.selected_vessel, self.selected_vessel["type"]["AIS_range"])
    ...
    def generate(self, coordinates_1: list[float], coordinates_2: list[float], current_route_index, density: int = 5, noise: float = 0.05):
        ...
        for _ in range(density):
            ...
            metadata = {
                ...
                "lon": rand_point[0],
                "lat": rand_point[1],
                "type": POSSIBLE_TYPES[math.floor(random.random() * len(POSSIBLE_TYPES))],
