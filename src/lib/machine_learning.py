    ...
    def next_state(self, new_closest_vessels: list[Vessel]) -> tuple[list[Vessel], list[Vessel]]:
        ...
        for x in suspects:
            if self.clf.predict([[
                ...
                self.possible_dark_activities.append(dataclasses.replace(x))
            else:
                self.possible_out_of_range.append(dataclasses.replace(x))

        self.selected_vessel_previous_tick = dataclasses.replace(self.selected_vessel)
        self.previous_closest_vessels = Util.deep_copy(new_closest_vessels)
